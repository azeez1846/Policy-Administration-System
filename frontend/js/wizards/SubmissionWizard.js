    ...
    container.innerHTML = `
        <div class="gw-panel">
            <div class="gw-panel-header" style="display:flex; justify-content:space-between; align-items:center;">
                <span>Location #1: Primary Commercial Facility</span>
                <button class="gw-btn gw-btn-primary" style="font-size:11px; padding:3px 10px;" onclick="triggerHazardHubWizardEnrichment()">⚡ HazardHub Risk Pre-Fill</button>
            </div>
            <div class="gw-panel-body">
                <div id="wizard-hazardhub-card" style="display:none; background:#0F172A; color:#F8FAFC; padding:12px; border-radius:6px; margin-bottom:14px; border-left:4px solid #38BDF8;">
                    <div style="font-weight:700; font-size:12px; color:#38BDF8; margin-bottom:4px;">⚡ HazardHub Risk Intelligence Enriched</div>
                    <div id="wizard-hazardhub-details" style="font-size:11px; color:#CBD5E1;"></div>
                </div>
...
async function triggerHazardHubWizardEnrichment() {
    try {
        const res = await fetch(`${API_BASE}/marketplace/hazard/enrich`, {
            ...
            body: JSON.stringify({
                locationId: 'accloc-1',
                buildingId: 'bldg-1',
                addressLine: '100 Industrial Parkway, Chicago, IL',
                state: 'IL'
            })
        });
        const data = await res.json();
        const card = document.getElementById('wizard-hazardhub-card');
        const details = document.getElementById('wizard-hazardhub-details');
        if (card && details && data.hazardIntelligence) {
            const h = data.hazardIntelligence;
            details.innerHTML = `🔥 Wildfire Risk: <strong>${h.wildfireScore}/100</strong> | 🌊 Flood Zone: <strong>${h.floodZone}</strong> | 🏠 Roof Rating: <strong>${h.roofConditionScore}/5.0</strong> | Category: <span style="color:#60A5FA;">${h.riskCategory}</span>`;
            card.style.display = 'block';
        }
        alert('⚡ HazardHub Property Risk Pre-Fill Completed!');
    } catch (e) {
        alert('HazardHub pre-fill failed: ' + e);
    }
}
